...
const CalculateScores = async (displayData, ori) => {
  ...
  for (let element of displayData) {
    const weatherScore = await CalculateWeatherScore(
      element,
      airData,
      psiData,
      rainfallData,
    ...
    if (
      element &&
      distanceScoresIndex.length !== 0 &&
      distanceScoresIndex.includes(element.index)
    ) {
      overallScores[element.index] =
        distanceScores[element.index] * DISTANCE_WEIGHTAGE +
        weatherScore * WEATHER_WEIGHTAGE;
    } else {
...
const CalculateWeatherScore = async (
  element,
  airData,
  psiData,
  rainfallData,
  ...
  if (sports.includes("(indoor)")) {
    // no UVI and Rainfall
    return (
      CalculateAirTempScore(air_temp) * 0.15 +
      CalculatePSIScore(PSI) * 0.15 +
      100 * 0.35 +
      90 * 0.35
    );
  } else {
    return (
      (CalculateAirTempScore(air_temp) +
        CalculatePSIScore(PSI) +
        CalculateRainfallScore(rainFall) +
        CalculateUVScore(UVI)) *
      EACH_API_WEIGHTAGE
    );
  }
};
...
const CalCulateDistanceScore = (distances, minDistance) => {
  ...
  for (let placeID in distances) {
    scores[placeID] =
      Math.exp((minDistance - distances[placeID]) / 12000) * 100;
  }
  return scores;
};
